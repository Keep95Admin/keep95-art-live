...
export default function CollectorAuth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignup) {
      ...
      const { data, error: signupError } = await supabase.auth.signUp({
        email,
        password,
        options: { data: { role: 'collector', username } },
      });
      if (signupError) {
      ...
      if (data.user) {
        ...
        if (!response.ok) {
          setError('Failed to create collector profile');
          setLoading(false);
          return;
        }
      }
  ...
  return (
    <main className="min-h-screen bg-black text-white flex items-center justify-center p-8">
      <form onSubmit={handleSubmit} className="max-w-md w-full space-y-6">
        <h1 className="text-4xl font-black text-center mb-8">Collector {isSignup ? 'Signup' : 'Login'}</h1>
        <input
          type="email"
          placeholder="Email"
          value={email}
